...
While drawing a `rectangle` or `ellipse`, hold `Shift` to constrain it to a
square/circle, or `Alt` to size it outward from the center point.
...
Hold `Shift` while dragging a resize handle to preserve the selection's
