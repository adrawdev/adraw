---
title: "Solid"
description: Documentation for SolidJS
---
...
Wrap your canvas UI in a `CanvasProvider` and render a `Canvas`.
...
To render multiple independent canvases, use multiple `CanvasProvider`/`Canvas` pairs:
...
Creates and owns an `AdrawCanvas` instance, scoped to its children via SolidJS context.
...
Renders the container element and mounts the `AdrawCanvas` from the nearest `CanvasProvider`.
